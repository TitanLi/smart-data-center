class MongoDB {
        ...
        this.cameraPower = function (col) {
            return new Promise(function (resolve, reject) {
                ...
                collection.find().sort({ datetime: -1 }).limit(2).toArray(function (err, items) {
                    resolve(items[0].camera_power_consumption - items[1].camera_power_consumption);
                });
            });
        }
    }
    ...
    async yesterdayAvgPowerRobot() {
        ...
        let camera_power_consumption = await this.cameraPower('cameraPowerLog');
        let yesterdayPower = {
            ...
            'cameraPower': (camera_power_consumption).toFixed(2)
        };
