class MongoDB {
        ...
        this.cameraPower = function (col) {
            return new Promise(function (resolve, reject) {
                ...
                collection.find().sort({ datetime: -1 }).limit(2).toArray(function (err, items) {
                    if(items <=1){
                        let cameraPowerData = {
                            cameraPowerConsumption : items[0].camera_power_consumption,
                            cameraPower : items[0].camera_power_consumption
                        }
                        resolve(cameraPowerData);
                    }else{
                        let cameraPowerData = {
                            cameraPowerConsumption : items[0].camera_power_consumption - items[1].camera_power_consumption,
                            cameraPower : items[0].camera_power_consumption
                        }
                        resolve(cameraPowerData);
                    }
    ...
    insertCameraPower(cameraPowerData) {
        return new Promise(function (resolve, reject) {
            ...
            let insertData = {
                "datetime": new Date(),
                "camera_power_consumption": cameraPowerData
            };
